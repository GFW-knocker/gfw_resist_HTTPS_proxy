...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("pyprox.log"), logging.StreamHandler()],
)
listen_PORT = 4500  # pyprox listening to 127.0.0.1:listen_PORT

num_fragment = (
    87  # total number of chunks that ClientHello devided into (chunks with random size)
)
...
class DNSOverFragment:
    def __init__(self):
        ...
        self.fragment_proxy = {"https": "http://127.0.0.1:" + str(listen_PORT)}
    ...
    def query(self, server_name):
        ...
        logging.info(f"online DNS Query", server_name)
        try:
            ...
            if (
                ...
                for x in answer_msg.answer:
                    if x.rdtype == dns.rdatatype.A:
                        ...
                        logging.info("################# DNS Cache is : ####################")
                        logging.info(
                            DNS_cache)  # print DNS cache , It's useful to track all resolved IPs , to be used later.
                        logging.info("#####################################################")
                        break
...
class ThreadedServer(object):
    ...
    def listen(self):
        ...
        while True:
            ...
            thread_up.daemon = True  # avoid memory leak by telling os its belong to main program , it's not a separate program , so gc collect it when thread finish
            thread_up.start()
    ...
    def handle_client_request(self, client_socket):
        ...
        if data[:7] == b"CONNECT":
            ...
            logging.info("redirect http to HTTPS", q_url)
            response_data = (
                    "HTTP/1.1 302 Found\r\nLocation: "
                    + q_url
    ...
    @staticmethod
    def extract_servername_and_port(data):
...
if __name__ == "__main__":
    start_log_writer()
    logging.info("Now listening at: 127.0.0.1:" + str(listen_PORT))
